...
- Corpus-wide `lint:batch` runs report three budgets as breached that are not: Recorded openers and "You have been X-ing" are per-batch allowances in VOICE-SPEC, and a status group six batches wide sums them. The per-batch use is the real one; the tool is not going to learn about batch boundaries.
- cups-05 and cups-06 slot 2 are the deck's only two reading lines with no sentence of six words or fewer. Both are accepted batch-1 copy with 7-word near-fragments, and cups-05 is the heaviest grief card with an author-edited question, so neither gets altered on a linter's say-so.
- The §5 blind re-rank is a consistency check, not a quality judgement: I wrote both sets and recognised the specimens on sight. It shows no drift between batch 1 and batch 6 (mean rank 10.2 against 10.8) and nothing more than that.
- The orphaned-definite-reference check prints rather than gates. At usable precision it flags 36 of 468 questions and 5 survive a careful read, and a gate with an 86 percent false-positive rate teaches people to ignore the tool.
- Opening-move classifiers must read the first two sentences, not the first. Reading one sentence put contrast at 3 percent against a 20 percent target; reading two puts it at 12 percent. In this voice a contrast opening routinely lands on sentence two ("Sending it changes two lives. Not sending it changes one.").
- Spread lines are 18–30 words, not 35–52 like reading lines: three are joined into one paragraph under three plates on a 390px screen, and 40 each would be a 120-word wall.
- The spread's single question comes from the Direction card's existing `questions`, which is why the stretch goal is 234 lines and not 312.
